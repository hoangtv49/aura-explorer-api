...
export class Cw20TokenController {
  ...
  @Post('get-by-denoms')
  @ApiOperation({ summary: 'Get list cw20 tokens by denoms' })
  @ApiResponse({ status: HttpStatus.OK })
  @UseInterceptors(ClassSerializerInterceptor)
  async getCw20TokenByDenoms(
    @ReqContext() ctx: RequestContext,
    @Body() request: Cw20TokenByDenomsParamsDto,
  ): Promise<any> {
    this.logger.log(ctx, `${this.getCw20TokenByDenoms.name} was called!`);
    const data = await this.cw20TokenService.getCw20TokenByDenoms(ctx, request);
